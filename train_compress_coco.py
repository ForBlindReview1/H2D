...
class Compress(torch.nn.Module):
    ...
    def data_parallel(self,ids):
        ...
        self.predict = torch.nn.DataParallel(self.predict,device_ids=ids)
...
def main(opt):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        save_file = os.path.join(opt.checkpoint_path, 'ckpt_epoch{epoch}_compress.pth'.format(epoch=epoch))
