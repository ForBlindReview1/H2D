...
# paths
parser.add_argument('--checkpoint_path', default='./output_byol/', type=str,
...
class BYOL(torch.nn.Module):
    def __init__(self,key_model,query_model,
                 hidden_dim=1024,feat_dim=12544, m=0.99):
...
def train_epoch(epoch, loader, model, criterion, optimizer, opt):
    ...
    for idx, (k_img, q_img, k_bbox, q_bbox) in enumerate(loader):
        ...
        q, k = model(k_img, q_img, k_bbox, q_bbox)
        loss = criterion(q, k)
...
def main(opt):
    ...
    model = BYOL(key_model,query_model,
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if epoch != 0 and epoch % opt.save_freq == 0 or epoch == opt.epochs-1:
            print('==> Saving...')
            save_file = os.path.join(opt.checkpoint_path, 'ckpt_epoch_{epoch}_lr_{lr}_samp{samp}.pth'.format(epoch=epoch,lr=opt.learning_rate,samp=opt.samp))
            # saving the model
        ...
        save_file = os.path.join(opt.checkpoint_path, 'ckpt_epoch{epoch}_byol.pth'.format(epoch=epoch))
    ...
    if opt.save_fig:
        ...
        save_path = os.path.join(opt.checkpoint_path, 'loss_epoch_{epoch}_lr_{lr}_samp{samp}.png'.format(epoch=epoch,lr=opt.learning_rate,samp=opt.samp))
